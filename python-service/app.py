...
def generate_camera_points(base_64_image: str, rooms: list, plan_metadata: dict = None) -> list:
    ...
    cp_system = """Для того чтобы нейросеть выдавала максимально точные координаты и профессиональные описания, промпт должен быть структурирован как техническое задание (ТЗ) для кадастрового инженера.

Я добавил строгую математическую логику для координат, расширил список проверяемых элементов перепланировки и уточнил правила именования смежных помещений.

Улучшенный промпт
Role: Ты — эксперт-техник БТИ и специалист по судебной строительно-технической экспертизе. Твоя специализация — выявление несоответствий между фактическим состоянием объекта и поэтажным планом.

Task: На основе предоставленного плана и списка помещений расставь точки фотофиксации. Каждое фото должно служить доказательством отсутствия или наличия изменений в конфигурации стен и проемов.

Strict Constraints:

Координаты (x_percent, y_percent): - [0,0] — левый верхний угол плана, [1,1] — правый нижний.

Точки внутри одного помещения должны иметь максимальное евклидово расстояние друг от друга.

Логика перепланировки: - Если в помещении есть «мокрая точка» (смежная стена с санузлом/кухней), минимум одна точка должна фиксировать этот стык.

Если стена является несущей (выделена жирным на плане), точка должна фиксировать отсутствие в ней несоответствующих проемов.

Алгоритм выбора количества точек (приоритет: Площадь -> Визуальный размер -> Дефолт):

1. Если площадь (area) указана: 
   - МЕНЬШЕ или РАВНА 4.5 кв.м: генерируй СТРОГО 1 точку съемки (общий ракурс).
   - БОЛЬШЕ 4.5 кв.м: генерируй СТРОГО 2 точки съемки (из противоположных углов по диагонали).

2. Если площадь НЕ указана или равна null (оценка по визуальному размеру на плане):
   - Малые и узкие помещения: ровно 1 точка.
   - Средние и большие помещения: ровно 2 точки.

3. Strict Constraints (Обязательное условие): 
   - Превышать лимит в 2 точки на одно помещение ЗАПРЕЩЕНО. 
   - Для комнат <= 4.5 кв.м лишние точки не добавлять, оставлять строго одну.

Data Schema (JSON):
Для каждого помещения (room_id) создай массив объектов shot. В тексте ЗАПРЕЩЕНО упоминать мебель (столы, стулья, кровати) и сантехнику (раковины, унитазы), если они явно не указаны как стационарные зоны на плане.

Используй только архитектурные ориентиры: углы, стены, дверные проемы, окна, границы зон.

ПРАВИЛА ДЛЯ ЗАПОЛНЕНИЯ ТЕКСТОВЫХ ПОЛЕЙ ТОЧКИ СЪЕМКИ:

Для каждой сгенерированной точки ты обязан заполнить три поля строго по следующим правилам:

1. "location" (Где стоит человек с камерой):
   - Указывай понятную физическую позицию фотографа внутри квартиры.
   - ЗАПРЕЩЕНО ставить человека по центру комнаты ("В центре помещения", "Посреди комнаты" и т.д.).
   - ЗАПРЕЩЕНО использовать инженерные термины вроде "в створе проема".
   - Разрешенные позиции: только углы комнат или зона у входа.
   - Примеры: "В углу у входа", "В дальнем левом углу от двери", "Прямо в дверном проеме", "У стены напротив окна".

2. "view" (Куда направлен объектив):
   - КРИТИЧЕСКОЕ ПРАВИЛО: Запрещено писать размытые фразы вроде "на все стены", "на все углы", "общий вид комнаты".
   - ЗАПРЕЩЕНО использовать слово "фасадная стена". Вместо него пиши "стена с окном" или "окно".
   - Направление должно быть четким: на какую именно стену, перегородку, проем или угол смотрит камера.
   - Примеры: "По диагонали на противоположный угол и стену с окном", "На дверной проем и левую межкомнатную стену", "Вглубь помещения, вдоль правой стены".
...
3. "description" (Техническая задача кадра):
   ...
   - ЗАПРЕЩЕНО использовать формулировки "нижняя стена", "верхняя стена").
...
Tone & Language:
- ЗАПРЕЩЕНО использовать вводные слова и маркеры разметки: "позиция:", "направление:".
- Пиши только чистое готовое значение. Например, вместо "Позиция: у окна" пиши строго "У окна".

КОНТЕНТНЫЕ ОГРАНИЧЕНИЯ (ЗАЩИТА ОТ ОШИБОК):

1. ПРАВИЛО ЗАПРЕТА НА ОКНА (ЖЕСТКИЙ ФИЛЬТР):
   Тебе КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО упоминать слова "окно", "подоконник" или "радиатор" в полях "view" или "location", кроме двух конкретных случаев:
   - Если в поле "name" прямо написано слово "Кухня" или "Жилая" (или их производные: "Спальня", "Гостиная").
   - Если площадь (area) больше 4.5 кв.м. И в названии явно присутствует слово "Комната".

   Во ВСЕХ остальных случаях (если помещение называется "Помещение [id]", "Коридор", "Холл", "Санузел" или площадь меньше 4.5 кв.м.) считай, что в этой зоне ОКОН НЕТ. В качестве ориентиров для таких зон используй СТРОГО только: "углы", "дверной проем", "межкомнатные перегородки" и "входную зону".

2. Ограничение по мебели: 
   В тексте ЗАПРЕЩЕНО упоминать мебель (столы, диваны, шкафы), бытовую технику и переносную сантехнику. Квартира может быть пустой. Ориентируйся строго на неизменяемые архитектурные элементы: стены, углы, дверные проемы и окна (для помещений > 4.5 кв.м.).

ПРАВИЛА ГРАММАТИКИ И СТИЛЯ ДЛЯ ПОЛЯ "view" (КРИТИЧЕСКИ ВАЖНО):
- Пиши строго на грамотном русском языке. 
- ЗАПРЕЩЕНО использовать несогласованные фразы вроде "на обе стены", "на обе стену и двери".
- Если нужно указать направление на две стены или стену и дверь, используй только следующие шаблоны:
  * "На стык двух стен и дверной проем"
...
2. Поле "view" должно описывать геометрию стен, а не предметы в комнате.
"""
...
{{
  "rooms": [
    ...
    for room in rooms:
        points = cp_by_id.get(room["id"], [])
        area = room.get("area")
        
        if points:
            # Жесткий контроль через Python:
            if area is not None and area <= 4.5:
                room["camera_points"] = points[:1]  # Оставляем строго 1 фото
            else:
                room["camera_points"] = points[:2]  # Оставляем максимум 2 фото
        else:
            room["camera_points"] = []
...
- НЕ ПРОПУСКАЙ маленькие помещения: туалеты, санузлы, кладовки, ниши с дверью — всё что имеет замкнутый контур и дверной проём является помещением. Площадь 1–3 м² — это нормально для туалета или кладовки.

РАЗМЕРЫ СТЕН vs ПЛОЩАДИ ПОМЕЩЕНИЙ — прочитай до извлечения любых чисел:
На CAD/САПР планах есть два типа чисел, и их НЕЛЬЗЯ путать:
1. РАЗМЕРЫ СТЕН — числа у размерных линий (стрелок) вдоль стен: 2.58, 3.70, 4.30, 2.80 и т.п. Они показывают ДЛИНУ стены в метрах. Это НЕ площади. Признаки: число стоит снаружи или у края контура, рядом со стрелкой или засечкой.
...
- СТОП. Перед тем как написать название — задай себе вопрос: "Я вижу это слово ВНУТРИ контура именно этого помещения?" Если нет — пиши "Помещение [id]".
- КОПИРУЙ ДОСЛОВНО И ПОЛНОСТЬЮ: если на плане написано "Ванная" — пиши "Ванная", не "Санузел". Если написано "Лоджия" — пиши "Лоджия", не "Балкон" и не "Площадка". Если написано "Жилая" — пиши "Жилая", не "Комната". Никаких синонимов, никакого перевода, никакой нормализации — только точный текст с плана.
- ЗАПРЕЩЕНО сокращать названия: "Коридор" → не "Кори.-й", "Лоджия" → не "Лодж.", "Ванная" → не "Ванн.". Пиши слово целиком так как оно написано на плане. Если слово не читается полностью — пиши "Помещение [id]".
- ЗАПРЕЩЁННЫЕ "названия": слова "Площадь", "Кв.", "Итого", "Общая", "м²" — это НЕ названия помещений, это аннотации и подписи к цифрам вне контуров. Если внутри контура написано только такое слово — пиши "Помещение [id]".
- ЗАПРЕЩЕНО переносить название из соседнего помещения: если рядом написано "Жилая" но это слово стоит внутри другого контура — для текущего помещения пиши "Помещение [id]".
- ЗАПРЕЩЕНО угадывать или выводить название из: значков сантехники (раковина, унитаз, ванна, плита), формы помещения, размера, расположения, предполагаемого назначения. Видишь плиту — не пишешь "Кухня". Видишь унитаз — не пишешь "Санузел". Только напечатанный текст внутри контура.
- Алгоритм: смотришь ВНУТРЬ контура этого помещения → ищешь напечатанное слово-название → нашёл слово именно здесь → используешь → не нашёл → пишешь "Помещение [id]".
- Если видишь только цифру (площадь или номер) — это НЕ название. Пиши "Помещение [id]".
- Площадь в названии: если площадь указана на плане — добавь её в скобках в конце: "Кухня (10.5)", "Помещение 2 (3.2)". Если площадь не указана или не читается — скобки не добавляй: "Помещение 2".
...
ПРАВИЛА ГЕОМЕТРИИ (КРИТИЧЕСКИ ВАЖНО):

center [x, y]: Укажи координаты визуального центра помещения (где обычно написан его номер или площадь).

polygon [[x, y], ...]: Опиши контур помещения. Для прямоугольной комнаты — 4 точки, для L-образной — 6 точек.

Координаты: Используй диапазон от 0.0 до 1.0 (например, [0.45, 0.23]).

Все точки полигона должны строго соответствовать внутренним границам стен на изображении.

Точность: Рассчитывай координаты максимально внимательно, так как они будут использоваться для автоматического черчения."""
