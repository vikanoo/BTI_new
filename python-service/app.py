...
2. Если площадь НЕ указана или равна null (оценка по визуальному размеру на плане):
   - Малые и узкие помещения (санузел, совмещенный туалет, маленькая кладовая, тамбур): ровно 1 точка.
   - Средние и большие помещения (спальня, кухня, гостиная, коридор): ровно 2 точки.
...
1. ПРАВИЛО ОКРЫТЫХ/ГЛУХИХ ЗОН (ОКНА): Перед тем как упомянуть "окно" в полях view или location, посмотри на название и площадь помещения. В санузлах, ванных комнатах, туалетах, кладовых, коридорах, гардеробных и тамбурах ОКОН НЕТ И НЕ МОЖЕТ БЫТЬ. Упоминать слово "окно" в этих помещениях категорически ЗАПРЕЩЕНО. Используй в качестве ориентиров двери, углы и стены.
2. Ограничение по мебели: В тексте ЗАПРЕЩЕНО упоминать мебель (столы, диваны) и переносную сантехнику, ориентируйся только на архитектурные элементы.
