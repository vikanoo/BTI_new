...
def step_1_ocr_analysis(image_base64, target_area=None):
    ...
    prompt = (
        "Ты — робот-сканер БТИ. Твоя задача — ПЕРЕПИСАТЬ текст с картинки.\n\n"
        "СТРОГИЕ ПРАВИЛА ИМЕНОВАНИЯ:\n"
        "1. Если в помещении написан ТЕКСТ (например, 'кухня', 'жилая') — пиши этот текст.\n"
        "2. Если в помещении НЕТ текста, а только цифры или пусто — пиши СТРОГО слово 'Помещение'.\n"
        "3. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО угадывать название по мебели или сантехнике. "
        "Видишь унитаз, но нет слова 'санузел'? Пиши 'Помещение'. Видишь плиту, но нет слова 'кухня'? Пиши 'Помещение'.\n"
        "4. Исправляй только технические опечатки в существующих словах (например, 'коридорр' -> 'Коридор').\n\n"
        f"ДАННЫЕ: {area_hint}\n"
        "Верни JSON: {'is_plan': true, 'rooms': [{'id': '...', 'name': '...', 'area': float, 'shape': '...'}]}"
    )
...
def step_2_photo_planning(ocr_json):
    """Этап 2: Техническое ТЗ на съемку (СТРОГО НА РУССКОМ ЯЗЫКЕ, БЕЗ ВОДЫ)"""
    headers = {"Authorization": f"Bearer {OPENAI_API_KEY}", "Content-Type": "application/json"}

    prompt = (
        f"На основе данных БТИ: {ocr_json}\n\n"
        "Ты — технический инструктор для фотографов. Составь план съемки.\n"
        "КАТЕГОРИЧЕСКИЕ ТРЕБОВАНИЯ:\n"
        "1. ЯЗЫК: Весь текст (location, view) должен быть СТРОГО НА РУССКОМ ЯЗЫКЕ.\n"
        "2. ЗАПРЕЩЕННЫЕ СЛОВА: 360 градусов, панорама, уютный, мебель, стороны света (север, юг и т.д.), детали, интерьер, атмосфера.\n"
        "3. ОРИЕНТИРЫ: Используй только: дверной проем, угол слева от входа, угол справа от входа, центр стены, окно, противоположная стена.\n"
        "4. КОЛИЧЕСТВО: Минимум 2 точки для простых комнат, 3-4 для сложных (Г-образных).\n\n"
        "СТРУКТУРА ОТВЕТА (JSON):\n"
        "- location: где физически стоит камера (на русском).\n"
        "- view: что именно в кадре (на русском).\n\n"
        "Пример: location: 'В дверном проеме', view: 'Вид на окно и левую часть комнаты'."
    )
