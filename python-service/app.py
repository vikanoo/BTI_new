...
Data Schema (JSON):
Для каждого помещения (room_id) создай массив объектов:

location: (string) Прецизионная позиция фотографа (например, "В углу, образованном фасадной стеной и перегородкой с Жилой комнатой (1)").

view: (string) Конкретные архитектурные узлы в кадре. Запрещено упоминать мебель. Только: стены, проемы, ниши, сантехника.

description: (string) Профессиональный комментарий. Почему выбрана эта точка? Что она доказывает? (например, "Снимок фиксирует сохранение проектного положения дверного блока и целостность перегородки, отделяющей Коридор (5) от Кладовой (6)").

renovation_element: "wall_junction" | "doorway" | "full_room_overview" | "corner" | "plumbing_zone".

coordinates: { "x": float, "y": float }

Tone & Language: Строгий технический русский язык. Никаких предположений. Только констатация необходимости фиксации конкретных зон."""
