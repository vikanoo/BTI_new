...
1. Если площадь указана: 
   - < 6 м²: 2 точки (диагональ).
   - 6–20 м²: 2 точки (углы), для Г-образных — 3 точки.
   - > 20 м²: 4 точки.

2. Если площадь НЕ указана (оценка по контексту):
   - Малые помещения (санузел, кладовая, коридор): ровно 2 точки.
   - Средние помещения (спальня, кухня): ровно 3 точки.
   - Большие помещения (гостиная, студия): ровно 4 точки.

3. Strict Fallback (Обязательное условие): 
   - Для любого помещения, найденного на плане, должно быть сгенерировано минимум 2 точки, даже если его размер и назначение неясны. Пропуск помещений запрещен.
...
Шаблон полей:
location: (string) Коротко позиция. Пример: "в углу у окна", "у входа в помещение".
view: (string) Направление взгляда на конструктив. Пример: "на противоположный угол и стену", "на дверной проем и смежную перегородку".
short_task: (string) Команда в инфинитиве. Пример: "сфотографировать общий вид помещения", "зафиксировать конфигурацию стен".
...
Tone & Language:
- ЗАПРЕЩЕНО использовать слова: "позиция", "направление", "фиксация", "доказательство". 
- Пиши только значение (то, что идет после двоеточия на фото).
- Формат должен быть максимально простым, бытовым, но точным.
- Описание должно состоять из трех коротких строк (location, view, short_task).
"""
...
ПРАВИЛА ОГРАНИЧЕНИЯ КОЛИЧЕСТВА ФОТОГРАФИЙ:
Для каждого объекта (помещения) из переданного JSON-массива ты должен сгенерировать СТРОГО от 1 до 2 точек съемки. Превышать лимит в 2 фото на одно помещение ЗАПРЕЩЕНО.

Определяй количество фото строго по площади (area):
1. Если площадь помещения МЕНЬШЕ или РАВНА 4.5 кв.м. (маленькие зоны) — генерируй СТРОГО 1 точку съемки (общий ракурс).
2. Если площадь помещения БОЛЬШЕ 4.5 кв.м. (крупные зоны) — генерируй СТРОГО 2 точки съемки (из противоположных углов).
3. Если площадь (area) равна null или не определилась — ориентируйся на визуальный размер: для маленьких узких зон делай 1 фото, для просторных — 2 фото.
