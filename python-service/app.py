...
def analyze_bti():
    ...
    prompt = """
    Проанализируй план БТИ. Твоя задача — составить задание на фотофиксацию для выявления перепланировок.
    
    1. ПРОВЕРКА: Если на фото не план БТИ, верни JSON: {"error": true, "message": "На фото не БТИ"}.
    
    2. ПРАВИЛА АНАЛИЗА ПОМЕЩЕНИЙ:
       - Извлеки 'id' (номер на плане) и 'name' (название строго по экспликации, иначе 'помещение').
       - Для каждого помещения определи количество точек съемки (от 2 до 4). 
       - Если форма помещения сложная (Г-образная, П-образная, наличие выступов), используй минимум 3-4 точки.

    3. ПРАВИЛА ОПИСАНИЯ ТОЧЕК (camera_points):
       - ЗАПРЕЩЕНО использовать мебель или предметы интерьера для ориентиров.
       - ИСПОЛЬЗУЙ ТОЛЬКО: углы, дверные проемы, окна, мокрые точки (раковины/унитазы на плане), вентиляционные короба и несущие стены.
       - 'location': описывай положение относительно входа или углов (например: "правый дальний угол от входа").
       - 'view': указывай, какие конструктивные элементы должны попасть в кадр (стыки стен, границы зон, положение дверного проема). Это нужно для сверки с планом БТИ на предмет переноса стен.
      ...
      "rooms": [
        {
          ...
          "camera_points": [
            {
              "point_id": 1,
              "location": "В дверном проеме",
              "view": "Вид на противоположную капитальную стену и примыкающие к ней перегородки"
            },
            {
              "point_id": 2,
              "location": "Дальний левый угол относительно входа",
              "view": "Вид на оконный проем и прилегающие углы для фиксации геометрии внешних стен"
            }
          ]
        }
      ]
