...
def step_1_ocr_analysis(image_base64, target_area=None):
    ...
    prompt = (
        "Ты — технический сканер. Твоя задача: выдать JSON со списком комнат.\n"
        f"ЦЕЛЕВАЯ ПЛОЩАДЬ: {target_area if target_area else 'Не указана'}\n\n"
        "ПРАВИЛА:\n"
        "1. ЯЗЫК: Пиши названия СТРОГО на русском языке (кириллица). Никаких спецсимволов и кодов типа \\u000.\n"
        "2. НАЗВАНИЯ: Если на плане написано 'Кухня' — пиши 'Кухня'. Если названия нет — пиши 'Помещение'.\n"
        "3. ЦИФРЫ: Ищи площади (числа с 'м2' или под чертой). Игнорируй размеры стен.\n"
        "4. ПРОВЕРКА: Сумма площадей всех комнат должна быть максимально близка к ЦЕЛЕВОЙ ПЛОЩАДИ.\n\n"
        "СТРУКТУРА JSON:\n"
        "{\n"
        "  'is_plan': true,\n"
        "  'rooms': [{'id': 'номер или null', 'name': 'название', 'area': float}]\n"
        "}"
    )
...
def critic_verification(image_base64, step1_json, target_area=None):
    ...
    prompt = (
        "Ты — проверяющий инспектор. Исправь ошибки в JSON, основываясь на картинке.\n"
        f"ДАННЫЕ OCR: {step1_json}\n"
        f"ОЖИДАЕМАЯ СУММА: {target_area}\n\n"
        "ЧТО ПРОВЕРИТЬ:\n"
        "1. МУСОР В ТЕКСТЕ: Если в 'name' видишь странные коды (\\u000), замени их на нормальные русские названия с картинки.\n"
        "2. ПРОПУСКИ: Если на картинке комнат больше, чем в JSON — добавь их.\n"
        "3. ЛИШНЕЕ: Если сумма комнат в JSON намного больше ОЖИДАЕМОЙ СУММЫ, значит в список попали лишние цифры (размеры стен). УДАЛИ ИХ.\n"
        "4. ИТОГ: Выдай исправленный JSON. Никаких пояснений, только JSON.\n\n"
        "ОТВЕТЬ В JSON:\n"
        "{\n"
        "  'corrected_json': { ... },\n"
...
def bti_endpoint():
    ...
    try:
        ...
        # Чистим имена комнат от мусора и перезаписываем
