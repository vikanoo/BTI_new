...
Data Schema (JSON):
Для каждого помещения (room_id) создай массив объектов со строго лаконичным содержанием:

location: (string) Начинается с "Позиция: ". Краткое описание места (например, "Позиция: в углу у двери").

view: (string) Начинается с "Направление: ". Перечисление ключевых узлов через союз "и" (например, "Направление: на раковину и душевую кабину").

description: (string) Цель съемки в 3-5 словах без вводных фраз (например, "сфотографировать весь санузел" или "фиксация проема в несущей стене"). ЗАПРЕЩЕНО писать профессиональные комментарии и обоснования.

renovation_element: "wall_junction" | "doorway" | "full_room_overview" | "corner" | "plumbing_zone".

coordinates: { "x": float, "y": float }

Tone & Language: 
- Предельно краткий стиль. 
- Исключить слова: "доказывает", "фиксирует", "проектное положение", "целостность". 
- Использовать только простую констатацию места и направления, как в техническом журнале осмотра.
"""
