...
- [ ] Провести полный тест с реальным планом БТИ (3-комнатная квартира) — ручной шаг после деплоя python-service на VPS
- [ ] Проверить сценарий с PDF-планом — ручной шаг
- [ ] Проверить уведомление менеджера при низкой уверенности — ручной шаг

**Технические доработки**

- [x] Исправить сохранение `plan_url`: `Update a row1` использовал `.item` → исправлено на `.first()`
- [x] Полноразмерное фото: `Code - Extract File ID` использует `photo[photo.length - 1]` — реализовано ранее
- [x] Обработка ошибок Gemini: `continueOnFail: true` + `IF - Gemini Error` → сообщение пользователю + сброс сессии
- [x] Обработка ошибок GPT-4o: `continueOnFail: true` + `IF - GPT4o Error` → сообщение пользователю + сброс сессии
- [x] Тайм-аут сессии: `IF - Session Timeout` после `Get a row1` — `updated_at` старше 48 ч → сброс + уведомление
- [x] Тексты сообщений бота проверены при реализации
- [x] Текст вне очереди при `collecting_photos`: `IF - Text During Photo Collection` → "Ожидаю фотографию, для отмены /start"

**Документация**

- [x] Обновлён `CLAUDE.md`: Python-сервис, архитектура, статус, ключевые файлы
- [x] Инструкция по `GEMINI_API_KEY`, `MANAGER_CHAT_ID`, `PYTHON_SERVICE_URL` добавлена в `CLAUDE.md`

🖍️**Заметки:** Добавлено 11 узлов в N8N (3 для timeout, 3 для Gemini error, 3 для GPT4o error, 2 для text-during-photos). Сквозное тестирование — ручной шаг, требует деплоя python-service на VPS.
...
| ✅ Вых 28–29.03 | Технические доработки выполнены; сквозной тест — ручной шаг после деплоя VPS |
